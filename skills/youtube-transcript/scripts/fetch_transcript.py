...
def main():
    ...
    # Try direct fetch first (no VPN), fallback to VPN
    try:
        api_direct = YouTubeTranscriptApi()
        transcript_raw = api_direct.fetch(video_id, languages=languages)
        transcript = [{"text": entry.text, "start": entry.start, "duration": entry.duration} for entry in transcript_raw]
    except Exception:
        # Fallback: try VPN
        ...
        if not vpn_ok:
            ...
            if not vpn_ok:
                print(json.dumps({"error": f"Direct fetch failed and VPN not available: {vpn_msg}"}))
                sys.exit(1)
